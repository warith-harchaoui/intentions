...
THRESHOLDS: dict[str, EngineThreshold] = {
    # TF-IDF: cheap and surface-level. Solid on near-training phrasings.
    "tfidf": {"min_accuracy": 0.75, "max_latency_ms": 50.0},
    # BERT: should beat TF-IDF on paraphrases; latency dominated by the
    # embedding pass (SBERT on CPU or the Ollama embedder).
    "bert": {"min_accuracy": 0.80, "max_latency_ms": 2000.0},
    # LLM: highest accuracy expected, but slow — seconds per call on a
    # local model, hence the multi-second ceiling.
    "llm": {"min_accuracy": 0.85, "max_latency_ms": 40000.0},
}
